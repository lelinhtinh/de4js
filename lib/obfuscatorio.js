...
function ObfuscatorIO(source, options) {
  const detectPattern = /((?![\d\W])[\w$]*)\(-?('|")(0x[a-f\d]+|\\x30\\x78[\\xa-f\d]+)\2(\s*,\s*('|").+?\5)?\)/gi;
...
function decode({ headCode, mainCode }, options, detectPattern) {
  headCode = headCode.replace(/\b(const|let)(\s*(?![\d\W])[\w$]*=)/gi, 'var $2');
  eval(headCode);
  ...
  mainCode = utils.strMerge(mainCode);
  if (options.methodChain) mainCode = utils.methodChain(mainCode);
  mainCode = utils.cleanGrouping(mainCode);
...
function getFuncDefiner(key, source) {
  ...
  while (funcGroupPattern.test(source)) {
    ...
    const splitSource = (pos) => {
      ...
      const returnMatch = varFunc.match(/(return\s+((?![\d\W])[\w$]*))(?![\s\S]*return)/i);
      keyFunc.push({
        key,
